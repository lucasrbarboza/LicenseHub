...
# No formulário você poderá definir o ambiente (development, production) e configurar o token da API.
# Em `production` a autenticação da API é obrigatória; será gerado um token automaticamente caso você não informe um. O token gerado será exibido ao final da instalação — copie e guarde em local seguro.
# Em `development` a autenticação é opcional (útil para testes).
